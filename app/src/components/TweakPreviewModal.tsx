...
export function TweakPreviewModal({ changes, description, onCancel, onConfirm, risk, title, trust }: TweakPreviewModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/20 px-4 backdrop-blur-md">
      <div className="w-full max-w-2xl rounded-[2rem] bg-surface p-6 shadow-float ring-1 ring-inset ring-border/65">
        ...
        <div className="mt-5 rounded-[1.5rem] bg-surface-muted px-5 py-4 text-sm text-muted">
          Risk level: <span className="font-medium capitalize text-text">{risk}</span>. A rollback snapshot will be created before anything changes.
        </div>
        <div className="mt-4 rounded-[1.5rem] bg-surface-muted/70 px-5 py-5">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          <div className="rounded-[1.5rem] bg-surface-muted/70 px-5 py-4">
        ...
        {trust.blocking_reason ? (
          <div className="mt-4 rounded-[1.5rem] bg-surface-muted px-5 py-4 text-sm leading-6 text-text">
        ...
        <div className="mt-6 flex justify-end gap-3">
          ...
          <button className="button-primary" disabled={!confirmed || Boolean(trust.blocking_reason)} onClick={onConfirm} type="button">
            Apply tweak
          </button>
        </div>
      </div>
